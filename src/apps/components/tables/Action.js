...
import { onDeploy } from "../forms/Item/ItemAddFormFields";
...
export const FormAction = ({ page, data, statusList, endpoint, flag }) => {
    ...
    function onClicked(id){
        const values = {
            userId:get_cookies.id,
            privateKey:get_cookies.walletKey,
            _buyerAddr:get_cookies.walletAddress,
            productId:id

        }

        pageLoader.loadingState(true)
        onDeploy(values).then(res => {
            ...
            <div className="dropdown dropdown-action">
                ...
                <div className="dropdown-menu dropdown-menu-right" x-placement="bottom-end">
                    ...
                    <button onClick={() => onClicked(data.row.values.id)} className="dropdown-item">
                        <i className="fa fa-pencil m-r-5"></i> Deploy
                    </button>

                    {/* {page == 'doctor' ? (
                        <NavLink to={{
                            pathname: `/admin/doctorInfo/${data.row.values.id}`,
                            state: data.row.values
                        }} className="dropdown-item">
                    ...
                    {/* <a onClick={deleteRow} data-id={`${id}`} data-status={`${status}`} className="dropdown-item">
                        <i className="fa fa-trash-o m-r-5"></i> Delete
                    </a> */}
